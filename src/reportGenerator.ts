...
export function printTerminalReport(report: StackerReport): void {
  ...
  console.log(chalk.dim('  Generated by Stacker  •  github.com/stacker-cli/stacker'));
  console.log();
}
...
function generateMarkdownReport(report: StackerReport): string {
  const { repoName, stack, stats, architecture, merged, reasoning } = report;
  const lines: string[] = [];
  ...
  lines.push('---');
  lines.push('*Generated by [Stacker](https://github.com/stacker-cli/stacker)*');
